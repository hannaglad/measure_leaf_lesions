...
# If you think infected area is missing from the lesions, decrease this value
# If you think too much non-infected area is being included in the lesions, increase this value
# Range : 1 - 40
UPPER_THRESHOLD = 4
...
def detect_lesions(image_path, out_path=0, save=True):
    ...
    blurred_for_leaf = cv2.erode(a_channel_blurred, kernel=(5,5), iterations=10)
    
    # To expand real leasions in leaf apply dilation to the inside of the leaf 
    dilated_in_leaf = cv2.dilate(a_channel_blurred, kernel=(9,9), iterations=10)
    ...
    a_channel_blurred = cv2.GaussianBlur(a_channel_blurred, (11,11), cv2.BORDER_DEFAULT)
...
# Write function that just performs the image pre-processing
def preprocess_image(image_path, save=False, out_path=None):
     # Read in image 
    image = cv2.imread(image_path, cv2.IMREAD_COLOR)
    ...
    # Save the image 
    if save:
        cv2.imwrite(out_path, unmasked_normalized)
    
    return unmasked_normalized
...
def ut(test_path):
    detect_lesions(test_path, save=False)
...
if __name__ == "__main__" : 
    ...
    for i in range(len(files)):
        ...
        percent = detect_lesions(files[i], out_path=im_outpath, save=True)
